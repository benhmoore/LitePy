...
class TestLiteModel(unittest.TestCase):
    ...
    def test_LiteQuery(self):
        ...
        assert Person.where("age").is_not_equal_to(30).all() == [person1, person3, person4]
        ...
        assert Person.where("age").is_greater_than_or_equal_to(30).all() == [person2, person3, person4]
        ...
        assert Person.where("age").is_less_than_or_equal_to(30).all() == [person1, person2]

        # Test the is_like() method
        assert Person.where("name").is_like("%il%").all() == [person3]

        # Test the is_not_like() method
        assert Person.where("name").is_not_like("%il%").all() == [person1, person2, person4]
    ...
    def test_complex_queries(self):
        ...
        with self.assertRaises(ValueError):
            query = Person.where("age").is_equal_to(25).and_where("name LIKE 'J%'").and_where("name = 'Billy'")
